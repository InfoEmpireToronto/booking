					...
					<b-col>
						<b-tabs v-model="tabIndex">
							...
							<b-tab title="View/Edit">
								<b-row style="margin-bottom:0px;">
									<b-col sm="4">
										<strong>Select practitioner:</strong><br/>
										<b-form-select v-model="practitioner" :options="appointmentGraphicTable.doctorOptions"/>
										<strong class="invisible">{{Practitioner}}</strong>
									</b-col>
									<b-col sm="8" class="text-center text-middle pt-2">
										...
										<div class="mx-4 arrow">
											<strong>{{appointmentGraphicTable.table.monthdisplay}}</strong> &nbsp;
								...
								<b-row style="margin-bottom:20px;">
									<b-col sm="12">
										<b>Legend: </b>
										<i class="fa fa-square legend" style="color: #6f69b0;"></i> Booked
										<i class="fa fa-square legend" style="color: #0e54a4"></i> Paid
										<i class="fa fa-square legend" style="color: #bcd8f9;"></i> Available
										<i class="fa fa-square legend" style="color: #f3f6e9;"></i> Unavailable
								...
								<b-row>
									...
									<b-col sm="12" md="12" lg="9">
											<span style="position: absolute; top:8%; left:52%; z-index:999" v-if="!appointmentGraphicTable.table.table">
												<i class="fa fa-spinner fa-spin" style="font-size:36px; color:#0e54a4;"></i>
											</span>
											<!-- Appointment table -->
											<table class="appointment-table table-hover">
												<tr class="text-center">
													...
													<template v-for="week in appointmentGraphicTable.table.week">
														<td :class="week.class"><b class="uppercase">{{week.day}}</b><br/>{{week.date}}</td>
													</template>
												</tr>
												<template v-for="days in appointmentGraphicTable.table.table">
													<tr>
														...
														<template v-for="timeslot in days.timeslot">
															<td :class="timeslot.class" class="link" :id="timeslot.elementID" @click.stop="loadModal(timeslot, days, $event.target)" ></td>
															<b-popover :target="timeslot.elementID" triggers="hover" delay="100">
																<template>
							...
							<b-tab title="Create an appointment">
								...
								<b-collapse id="accordion2" visible accordion="my-accordion">
									<b-card>
										<b-row>
											<template v-for="(item, index) in appointment.doctors">
									...
									<b-card>
										...
										<b-row>
											<b-col sm="12" v-if="appointment.treatmentOptions2" class="text-center">
												<b-button variant="primary" :disabled="!appointment.step4" @click="gotoStep4">Continue</b-button>
											</b-col>
										</b-row>
									</b-card>
								</b-collapse>
								...
								<b-button v-b-toggle.accordion5 variant="light" class="btn-toggle-title" :disabled="!appointment.step5">
									5. Confirm
								</b-button>
								<b-collapse id="accordion5" visible accordion="my-accordion">
									<b-card>
										<b-row>
											<b-col sm="12">
										...
										<b-row style="margin-top: 15px;">
											<b-col sm="12">
												<b-button variant="primary" :disabled="!appointment.step5" @click="addAppointment">Create</b-button>
								...
								<b-row>
									<b-table responsive :hover="paid.hover" :items.sync="paid.items" :fields="paid.fields" :sort-by.sync="paid.sortBy" 
										...
										<template slot="invoice" slot-scope="row">
											<b-button size="sm" class="mr-1" :href="row.item.invoice" target="_blank">
												View
											</b-button>
										</template>
									</b-table>
							...
							<b-tab title="Financial activity" @click="loadAppointmentData(6)">
								<b-row>
									<label for="startdate" class="col-sm-1">From</label>
									<b-col sm="3">
										<b-form-input v-model="financial.from" id="startdate" type="date" @keyup.native="UpdateFinancial()" @change="UpdateFinancial()"></b-form-input>
									</b-col>
									<label for="enddate" class="col-sm-1" class="text-center">To</label>
									<b-col sm="3">
										<b-form-input v-model="financial.to" type="date" @keyup.native="UpdateFinancial()" @change="UpdateFinancial()"></b-form-input>
									</b-col>
									<label for="f-doctor" class="col-sm-1">Doctor</label>
									<b-col sm="3">
										<b-form-select v-model="financialDoctor" :options="financial.doctorOptions" id="f-doctor"/>
									</b-col>
								</b-row>
								<b-row class="search-row">
									...
									<b-col sm="3">
										<b-form-select :options="financial.pageOptions" v-model="financial.perPage" id="page"/>
									</b-col>
								</b-row>
								<b-row>
									<b-table responsive :hover="financial.hover" :items.sync="financial.items" :fields="financial.fields" :sort-by.sync="financial.sortBy" 
										:current-page="financial.currentPage" :per-page="financial.perPage" :filter="financial.filter" @filtered="onFiltered6" 
										:sort-by.sync="financial.sortBy" :sort-desc.sync="financial.sortDesc">
									</b-table>
								</b-row>
									<b-row>
										<b-col md="4">
										...
										<b-col md="4" class="my-1">
											<b-pagination  v-if="financial.totalRows > financial.perPage" :total-rows="financial.totalRows" :per-page="financial.perPage" v-model="financial.currentPage" class="my-0" />
										</b-col>
										<b-col md="4" class="text-center">
										<strong>Total: </strong>${{financial.total}}
										</b-col>
									</b-row>
							</b-tab>
							<!-- End financial activity tab-->
							<!-- Search tab-->
							<!-- <b-tab title="Search">
								<b-row>
									<b-col class="text-center">
										<p>Search by doctor, patient or date</p>
									</b-col>
								</b-row>
								<b-container>
									<b-form @submit="searchAppointment" class="text-center" id="search-appt-form">
										<b-row>
											<b-col cols="12">
												<div class="form-group">
													<b-col sm="12" lg="1">
														<label for="search-doctor">Doctor</label>
													</b-col>
													<b-col sm="12" lg="3">
														<b-form-select v-model="search.doctor" :options="search.doctorOptions" id="search-doctor"/>
													</b-col>
													<b-col sm="12" lg="1">
														<label for="search-patient">Patient</label>
													</b-col>
													<b-col sm="12" lg="3">
														<b-form-input v-model="search.name" type="text" id="search-patient" placeholder="Search name" autocomplete="off"
															@keyup.native="searchPatient(3)" @focus.native="onFocus(3)"  @blur.native="search.display = 'none'">
													</b-form-input>
													<ul class="dropdown-menu" 
														:style="{display: search.display}">
														<li v-for="item in search.nameOptions">
															<a :value="item.id" @click="selectPatient(item, 3)" @mousedown.prevent>
																{{item.firstname}}  {{item.lastname}}
															</a>
														</li>
													</ul>
													...
													<b-col sm="12" lg="3">
														<b-form-input v-model="search.date" type="date" id="search-date"></b-form-input>
													</b-col>
												</div>
												<div class="form-group">
													<b-col sm="12" offset-md="3" md="6">
														<span style="line-height: 2.5;">{{search.message}}</span>
													</b-col>
													<b-col sm="12" md="3" class="text-right">
														<b-button form="search-appt-form" type="submit" variant="primary">Search</b-button>
													</b-col>
												</div>
											</b-col>
										</b-row>
									...
									<b-row>
										<b-col cols="12">
											<template v-if="search.format === 'table' && search.resultDisplay">
												<div class="form-group justify-content-end">
													<b-col sm="6" md="3" lg="2" class="text-center">
													<label for="page">Per page</label>
													</b-col>
													<b-col sm="6" md="3" lg="2">
														<b-form-select :options="search.pageOptions" v-model="search.perPage" id="page"/>
													</b-col>
												</div>
												<div class="form-group">
													<b-table responsive :hover="search.hover" :items.sync="search.items" :fields="search.fields" 
														:sort-by.sync="search.sortBy" :current-page="search.currentPage" :per-page="search.perPage" 
														:filter="search.filter" @filtered="onFiltered7" :sort-by.sync="search.sortBy" :sort-desc.sync="search.sortDesc">
														<template slot="status" slot-scope="row">
															{{statusFormatter(row.item.status)}}
														</template>
														<template slot="edit" slot-scope="row">
															<b-button size="sm" @click.stop="searchApptInfo(row.item, row.index, $event.target)" class="mr-1">
																{{row.item.status == 0 ? 'View' : 'Edit'}}
															</b-button>
														</template>
													</b-table>
												</div>
												<table-pagination :item="search"></table-pagination>
											</template>
											<template v-if="search.format == 'graphic' && search.resultDisplay">
												<table class="appointment-table table-hover">
													<tr class="text-center">
														<td width="85px"></td>
														<template v-for="week in search.table.week">
															<td :class="week.class">{{week.date}} {{week.day}}</td>
														</template>
													</tr>
													<template v-for="days in search.table.table">
														<tr>
															<td>{{days.time}}</td>
															<template v-for="timeslot in days.timeslot">
																<td :class="timeslot.class" class="link" :id="timeslot.elementID" @click.stop="searchInfo(timeslot, days, $event.target)" ></td>
																<b-popover :target="timeslot.elementID" triggers="hover focus" delay="100">
																<template>
																	<table class="info-table">
																		<tr><td><strong>Date:</strong></td><td>{{timeslot.date}}</td></tr>
																		<tr><td><strong>Practitioner:</strong></td><td>{{timeslot.doctor}}</td></tr>
																		<tr><td><strong>Status:</strong></td><td>{{timeslot.status}}</td></tr>
																		<tr v-if="timeslot.patient !== null"><td><strong>Patient:</strong></td><td>{{timeslot.patient}}</td></tr>
																		<tr v-if="timeslot.phone !== null"><td><strong>Phone:</strong></td><td>{{timeslot.phone}}</td></tr>
																		<template v-if="timeslot.treatment !== null" v-for="(item, index) in timeslot.treatment">
																			<tr><td><strong>{{ index == 0 ? 'Treatment:' : ''}}</strong></td><td>{{item.name}}</td></tr>
																		</template>
						...
						<b-modal id="modalAdd" @hide="resetModal" title="Add an appointment">
							<b-form @submit="addAppt" id="modal-appt-form-add">
								...
								<div class="form-group">
									<!-- <b-col class="text-center">
										<p>Enter name or email to select a patient</p>
									</b-col> -->
								</div>
								<div class="form-group">
						...
						<b-modal id="modalEdit" @hide="resetModal" title="Edit / Cancel appointment" size="lg">
							<b-form @submit="updateAppt" id="modal-appt-form-edit">
								<div class="form-group">
									...
									<b-col sm="9">
										<b-form-input type="date" v-model="modalAppointment.date" id="edit-date" @keyup.native="searchAvailableDoctor(1)" 
										@change.native="searchAvailableDoctor(1)" required></b-form-input>
									</b-col>
								</div>
								<div class="form-group">
									<b-col sm="3">
								...
								<template v-if="modalAppointment.next > 0" v-for="(item, index) in modalAppointment.treatment">
									<div class="form-group">
										<b-col sm="3" class="text-right" style="padding-right:0px;"><i class="fa fa-minus-circle"
										@click="modalAppointment.treatment.splice(index, 1)" title="Remove treatment"></i></b-col>
										<b-col sm="9">
							...
							<div slot="modal-footer" class="w-100 pb-4">
								<div class="form-group">
						...
						<b-modal id="modalFullEdit" @hide="resetModal" title="Edit / Cancel appointment" size="lg">
							<b-form @submit="fullUpdateAppt" id="modal-appt-form-full-edit">
									...
									<div class="form-group">
										...
										<b-col cols="2" sm="2" md="2" lg="1" class="text-right col-treatment-button">
											<i class="fa fa-minus-circle" style="font-size:36px;color:#dc3545;cursor: pointer" 
												@click="modalAppointment.treatment.splice(index, 1)" title="Remove treatment">
											</i>
										</b-col>
									</div>
								</template>
								...
								<template v-for="(product, index) in modalAppointment.products">
									<div class="form-group">
										...
										<b-col cols="2" sm="2" md="2" lg="1" class="text-right col-treatment-button" v-if="index > 0">
											<i class="fa fa-minus-circle" style="font-size:36px;color:#dc3545;cursor: pointer" 
												@click="modalAppointment.products.splice(index, 1)" title="Remove treatment">
											</i>
										</b-col>
									</div>
								</template>
								<div class="form-group">
									...
									<b-col sm="3" lg="2" class="text-right"><i v-if="!modalAppointment.payoff" class="fa fa-plus-circle" style="font-size:36px;color:#28a745;cursor: pointer" 
										@click="moreProduct" title="Add more product"></i></b-col>
								</div>
								<div class="form-group" v-for="(payment, index) in modalAppointment.editPayment">
									...
									<b-col v-if="index > 0" cols="2" sm="2" md="2" lg="1" order-sm="2" order="2" order-md="2" order-lg="3" class="sm-mobile-form-margin-bottom col-treatment-button text-right">
										<i class="fa fa-minus-circle" style="font-size:36px;color:#dc3545;cursor: pointer" 
											@click="modalAppointment.editPayment.splice(index, 1)" title="Remove payment">
										</i>
									</b-col>
								</div>
								<div class="form-group">
