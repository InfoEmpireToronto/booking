function load()
{
		...
		else
		{
			...
			.then(function (response)
			{
				if(response.data.success)
				{
					appointments.appointmentGraphicTable.table = response.data.appointmentable;
					appointments.appointmentGraphicTable.monday = response.data.monday;
					appointments.doctor = response.data.doctor;
				}
			})
			.catch(function (error) {
			console.log(error);
	...
	var appointments = new Vue({
		...
		watch:
		{
			...
			{
				if(oldVal != null)
				this.searchAvailableTime(val, 2, this.modalAppointment.appointment);
			},
		...
		methods:
		{
			...
			reset()
			{
				this.appointment.step2 = this.appointment.step3 = this.appointment.step4 = false;
						...
						if(response.data.success)
						{
							switch(section)
							{
								...
								case 2:
									...
									appointments.appointment.patientInput = response.data.name;
									break;
							}
						}
					})
			...
			resetModal()
			{
				...
				this.modalAppointment.patientInput = this.modalAppointment.phone = null;
				this.modalAppointment.note = this.modalAppointment.patientNameInput = this.modalAppointment.patientName = null;
			...
			updateTable(date)
			{
				loadmodalAppointment(this.doctor, date);
			},
			...
			updateStatus()
			{
				loadmodalAppointment(this.doctor, this.appointmentGraphicTable.monday);
			...
			selectTreatment(item)
			{
				...
				this.appointment.step3 = this.appointment.treatments.length > 0 ? true : false;
				this.appointment.step4 = false;
			},
			...
			showCalendar()
			{
				...
				this.calendarClass = (this.calendarClass == 'd-none'
					) ? 'd-block' : 'd-none';
			}
		}
	});
